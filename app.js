...
/** Escapa texto digitado pelo usuário antes de colocá-lo em innerHTML. */
...
/** Célula de CSV (separador ";"): protege textos com ; aspas ou quebra de linha. */
...
function limparFormulario(ids, chaveSecao) {
  ids.forEach((id) => {
    ...
    if (!el) return;
    el.value = "";
    el._rawMoeda = "";   // sem isso, os dígitos antigos voltam ao digitar de novo
...
function validarCampos(specs) {
  ...
  specs.forEach(({ id, rotulo, tipo, opcional }) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.classList.remove("invalid");
    ...
    // Campo opcional em branco é aceito (vale 0)
    if (opcional && texto === "") return;

    const vazio = texto === "";
    const valorNumerico = numeroCampo(el);
    if (vazio || !Number.isFinite(valorNumerico) || !regra.teste(valorNumerico)) {
      el.classList.add("invalid");
      const aceitaZero = tipo === "dinheiro" || tipo === "taxa";
      const msg = vazio
        ? "precisa ser preenchido" + (aceitaZero ? " (use 0 se não houver)" : "")
        : regra.msg;
      erros.push(`<b>${rotulo}</b> ${msg}`);
...
/** O mês N (contado a partir de hoje) cai em dezembro? Usa a mesma conta de dataAbreviada(). */
function ehDezembro(mesesAdiante, mesAtual = new Date().getMonth()) {
  return (mesAtual + mesesAdiante) % 12 === 11;
}
...
function simularMeta(cfg) {
  ...
  while (saldo < cfg.meta && meses < MESES_LIMITE) {
    ...
    if (cfg.extraAnual > 0 && ehDezembro(meses, cfg.mesAtual)) {
...
function aporteNecessario(meta, inicial, taxaAnual, meses) {
  ...
  // Se só os juros do saldo atual já bastam, o aporte necessário é zero (nunca negativo)
  return Math.max(0, ((meta - inicial * k) * i) / (k - 1));
}
...
function desenharGrafico(canvas, modelo) {
  ...
  if (hover && hover.mes !== undefined) {
    ...
    ctx.shadowBlur = 12;
    ctx.beginPath();
    if (ctx.roundRect) ctx.roundRect(bx, by, caixaW, caixaH, 9);
    else ctx.rect(bx, by, caixaW, caixaH);
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.stroke();
...
function gerarDownloadCSV(chave) {
  ...
  if (!pacote) return;
  const conteudo = "\uFEFF" +
    celulaCSV(pacote.titulo) + "\r\n\r\n" +
    pacote.linhas.map((linha) => linha.map(celulaCSV).join(";")).join("\r\n");

  const blob = new Blob([conteudo], { type: "text/csv;charset=utf-8" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = "unifinance-" + chave + ".csv";
...
function calcularSimulador() {
  const valido = exibirErros("#simErr", validarCampos([
    ...
    { id: "sExtra",  rotulo: "Aporte extra",    tipo: "dinheiro", opcional: true },
    { id: "sTaxa",   rotulo: "Rentabilidade",   tipo: "taxa" },
    { id: "sInfla",  rotulo: "Inflação",        tipo: "taxa", opcional: true }
  ]));
  if (!valido) return;
  ...
  registrarCSV("metas-mensal",
    `UniFinance PRO - Trajetoria: ${nome}`,
  ...
  $("#resSim").innerHTML = `
    <section class="card secao-resultado">
      <h3>🧭 Sua trajetória até "${escaparHTML(nome)}"</h3>
      ${avisoNaoConcluida}
...
function calcularQuantoGuardar() {
  ...
  const avisoSemAporte = aporte === 0 ? `
    <div class="card-soft info">
      ✅ Seu saldo atual, rendendo ${porcentagem(taxa)} ao ano, já chega a
      <b>${dinheiro(r.saldoFinal)}</b> em ${mesesParaTexto(meses)} — não precisa de novos aportes.
    </div>` : "";

  registrarCSV("quanto-guardar",
    `UniFinance PRO - Aporte necessario (${meses} meses)`,
...
function calcularJuros() {
  ...
  registrarCSV("juros-compostos",
    `UniFinance PRO - Projecao de ${meses} meses`,
...
function gerarLinhasPlanejamento() {
  ...
  for (let mes = 1; mes <= MESES_LIMITE && saldo < meta; mes++) {
    saldo *= (1 + i);
    saldo += aporte;
    if (extra > 0 && ehDezembro(mes)) saldo += extra;
    ...
    <table class="tabela-planejamento">
      ...
      <tbody>${dados.map((linha, index) => `
        <tr>
          ...
          <td><input class="planejamento-input" data-planejamento="${index}" data-campo="observacao" value="${escaparHTML(linha.observacao)}" placeholder="Ex: consegui guardar menos este mês"></td>
        </tr>`).join("")}</tbody>
    </table>`;
...
function iniciarAplicativo() {
  ...
  console.log(
    "%c💰 UniFinance PRO%c pronto! Tudo roda localmente no seu navegador.",
